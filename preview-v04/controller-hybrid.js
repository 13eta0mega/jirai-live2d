import { clamp01 } from '../src/avatar/lipsync.js';
import { QUAD_INDICES, QUAD_UVS, expressionForPreset, motionFor, radians, sourceSize } from '../src/mesh/controller-common.js';
import { createDeformedQuadNormalized, deformGrid } from '../src/mesh/grid.js';
import { mouthPartForEmotion } from '../src/mesh/face-rig.js';
import { renderMethods as baseRenderMethods } from '../src/mesh/controller-render.js';
...
export const hybridRenderMethods = {
  ...
  renderTransition(time) {
    ...
    baseRenderMethods.renderTransition.call(this, time);
    const sample = this.lastTransitionSample || state.sample;
    const weights = hybridReferenceWeights(sample.raw ?? sample.progress ?? p, Boolean(fromImage), Boolean(toImage));
    if (fromImage) this.drawGeneratedReference(this.transition.from, this.transition.fromIntensity, time, weights.from, this.emotionStartedAt);
    if (toImage) this.drawGeneratedReference(this.transition.to, this.transition.toIntensity, time, weights.to, this.transition.start);
  },
