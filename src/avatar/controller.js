...
const emptyAudioResult = () => ({
  rms: 0, low: 0, mid: 0, high: 0, noiseFloor: 0.018, open: 0, weight: 0, viseme: "CLOSED",
});
...
async function loadOptionalImages(entries) {
  const loaded = await Promise.all(entries.map(async ([key, path]) => {
    try {
      return [key, await loadImage(path)];
    } catch (error) {
      console.warn(`[Jirai] optional part skipped: ${key}`, error);
      return null;
    }
  }));
...
export class AvatarController {
  constructor(canvas, presets = FALLBACK_PRESETS) {
    ...
    if (!this.ctx) throw new Error("2D canvas context를 생성할 수 없습니다.");
    this.presets = presets; this.images = {}; this.parts = {};
    ...
    this.audioResult = emptyAudioResult(); this.lipSyncState = createLipSyncState();
    this.audioContext = null; this.microphoneStream = null; this.microphoneSource = null; this.microphoneEndedHandler = null; this.analyser = null; this.timeData = null; this.frequencyData = null;
    ...
    this.onPointerMove = (event) => this.updatePointer(event); this.onPointerLeave = () => { this.pointerTarget.x = 0; this.pointerTarget.y = 0; }; this.onWindowResize = () => this.resize();
    ...
    else { this.resizeObserver = null; window.addEventListener("resize", this.onWindowResize); }
  }
  ...
  async load() {
    ...
    this.parts = await loadOptionalImages(Object.entries(PART_FILES));
    this.resize(); this.scheduleNextBlink(now()); return this;
  }
  ...
  updatePointer(event) { const rect = this.canvas.getBoundingClientRect(); if (!rect.width || !rect.height) return; this.pointerTarget.x = clamp(((event.clientX - rect.left) / rect.width) * 2 - 1, -1, 1); this.pointerTarget.y = clamp(((event.clientY - rect.top) / rect.height) * 2 - 1, -1, 1); }
  ...
  settleTransition(time = now()) { if (!this.transition) return; const progress = clamp((time - this.transition.start) / this.transition.duration); if (progress >= 0.5) { this.currentEmotion = this.transition.to; this.currentIntensity = this.transition.toIntensity; } else { this.currentEmotion = this.transition.from; this.currentIntensity = this.transition.fromIntensity; } this.transition.resolve?.(); this.transition = null; this.emotionStartedAt = time; }
  ...
  resetAudioAnalysis({ preserveTarget = false } = {}) { this.lipSyncState = createLipSyncState(); this.audioResult = emptyAudioResult(); if (!preserveTarget) { this.mouthTarget = this.manualMouth; if (this.manualMouth <= 0.02) { this.setVisemeName("CLOSED"); this.mouthFormTarget = 0; } } }

  async reset() {
    this.settleTransition(now()); this.lipSyncTest = false; this.audioFeatures = null; await this.stopMicrophoneLipSync({ preserveMode: true }); this.lipSyncMode = "manual";
    this.manualMouth = 0; this.mouthTarget = 0; this.mouthFormTarget = 0; this.setVisemeName("CLOSED"); this.resetAudioAnalysis();
    this.currentEmotion = "neutral"; this.currentIntensity = 1; this.emotionStartedAt = now(); this.breathEnabled = true; this.blinkEnabled = true; this.blinkStart = -1; this.blinkLevel = 1; this.scheduleNextBlink(now()); this.pointerTarget = { x: 0, y: 0 };
  }
  ...
  setViseme(viseme, weight = 1) { const requested = String(viseme || "CLOSED").toUpperCase(); const key = VISEMES[requested] ? requested : "CLOSED"; const selected = VISEMES[key]; const w = clamp01(weight); this.setVisemeName(key); this.mouthTarget = clamp01(selected.mouthOpen * w); this.manualMouth = this.mouthTarget; this.mouthFormTarget = (selected.mouthForm || 0) * w; }

  setLipSyncTest(enabled) { this.lipSyncTest = Boolean(enabled); if (this.lipSyncTest) { if (this.lipSyncMode === "microphone") void this.stopMicrophoneLipSync({ preserveMode: true, preserveAnalysis: true }); this.audioFeatures = null; this.resetAudioAnalysis({ preserveTarget: true }); this.lipSyncMode = "test"; } else if (this.lipSyncMode === "test") { this.lipSyncMode = "manual"; this.resetAudioAnalysis(); } }

  async startMicrophoneLipSync() {
    if (!navigator.mediaDevices?.getUserMedia) throw new Error("이 브라우저는 마이크 입력을 지원하지 않습니다.");
    await this.stopMicrophoneLipSync(); let stream = null; let context = null;
    try {
      ...
      context = new AudioContextClass(); if (context.state === "suspended") await context.resume(); const source = context.createMediaStreamSource(stream); const analyser = context.createAnalyser(); analyser.fftSize = 2048; analyser.smoothingTimeConstant = 0.36; source.connect(analyser);
      const endedHandler = () => { if (this.microphoneStream === stream) void this.stopMicrophoneLipSync(); }; for (const track of stream.getTracks()) track.addEventListener?.("ended", endedHandler, { once: true });
      this.audioContext = context; this.microphoneStream = stream; this.microphoneSource = source; this.microphoneEndedHandler = endedHandler; this.analyser = analyser; this.timeData = new Uint8Array(analyser.fftSize); this.frequencyData = new Uint8Array(analyser.frequencyBinCount); this.audioFeatures = null; this.resetAudioAnalysis({ preserveTarget: true }); this.lipSyncTest = false; this.lipSyncMode = "microphone"; return true;
    } catch (error) {
      stream?.getTracks().forEach((track) => track.stop()); if (context && context.state !== "closed") { try { await context.close(); } catch {} }
      this.audioContext = null; this.microphoneStream = null; this.microphoneSource = null; this.microphoneEndedHandler = null; this.analyser = null; this.timeData = null; this.frequencyData = null; this.lipSyncMode = "manual"; this.resetAudioAnalysis(); throw error;
    }
  }

  async stopMicrophoneLipSync({ preserveMode = false, preserveAnalysis = false } = {}) {
    const source = this.microphoneSource; const stream = this.microphoneStream; const context = this.audioContext; const endedHandler = this.microphoneEndedHandler;
    this.audioContext = null; this.microphoneStream = null; this.microphoneSource = null; this.microphoneEndedHandler = null; this.analyser = null; this.timeData = null; this.frequencyData = null;
    try { source?.disconnect(); } catch {}
    if (stream) { for (const track of stream.getTracks()) { if (endedHandler) track.removeEventListener?.("ended", endedHandler); track.stop(); } }
    if (!preserveMode && this.lipSyncMode === "microphone") this.lipSyncMode = this.lipSyncTest ? "test" : "manual";
    if (!preserveAnalysis) this.resetAudioAnalysis();
    if (context && context.state !== "closed") { try { await context.close(); } catch {} }
  }

  setAudioFeatures(features) { if (this.lipSyncMode !== "external") { if (this.lipSyncMode === "microphone") void this.stopMicrophoneLipSync({ preserveMode: true, preserveAnalysis: true }); this.resetAudioAnalysis({ preserveTarget: true }); } this.lipSyncTest = false; this.audioFeatures = { rms: Math.max(0, Number(features?.rms) || 0), low: Math.max(0, Number(features?.low) || 0), mid: Math.max(0, Number(features?.mid) || 0), high: Math.max(0, Number(features?.high) || 0) }; this.lipSyncMode = "external"; }
  clearAudioFeatures() { this.audioFeatures = null; if (this.lipSyncMode === "external") { this.lipSyncMode = this.lipSyncTest ? "test" : "manual"; this.resetAudioAnalysis(); } }
  setBlinkEnabled(enabled) { this.blinkEnabled = Boolean(enabled); if (!this.blinkEnabled) { this.blinkStart = -1; this.blinkLevel = 1; } else this.scheduleNextBlink(now()); }
  ...
  updateBlink(time) { const preset = this.activePresetForBlink(time); if (!this.blinkEnabled || preset.blinkMode === "disabled") { this.blinkLevel = 1; return; } if (this.blinkStart < 0 && time >= this.nextBlinkAt) { this.blinkStart = time; this.blinkDuration = preset.blinkMode === "special" ? 430 : 330; } if (this.blinkStart >= 0) { const progress = clamp((time - this.blinkStart) / this.blinkDuration); if (progress < 0.34) this.blinkLevel = 1 - easeInOut(progress / 0.34); else if (progress < 0.49) this.blinkLevel = 0; else this.blinkLevel = easeOut((progress - 0.49) / 0.51); if (progress >= 1) { this.blinkStart = -1; this.blinkLevel = 1; this.scheduleNextBlink(time); } } }
  ...
  sampleLipSync(dt, seconds, preset) {
    ...
    if (this.lipSyncTest) { const carrier = 0.5 + 0.5 * Math.sin(seconds * 9.4); const phrase = 0.5 + 0.5 * Math.sin(seconds * 1.7 + 0.3); const open = clamp01(0.04 + carrier * phrase * 0.9); const sequence = ["A", "I", "U", "E", "O"]; const key = sequence[Math.floor(seconds * 4) % sequence.length]; const selected = VISEMES[key]; this.setVisemeName(open < 0.08 ? "CLOSED" : key); this.mouthTarget = open; this.mouthFormTarget = (selected.mouthForm || 0) * open; this.audioResult = { ...emptyAudioResult(), rms: open * 0.2, open, weight: open, viseme: this.viseme }; return; }
    this.mouthTarget = this.manualMouth;
  }

  update(time) { if (!this.lastTime) this.lastTime = time; const dt = Math.min(64, Math.max(1, time - this.lastTime)); this.lastTime = time; const seconds = time / 1000; const activeId = this.transition ? this.transition.to : this.currentEmotion; const preset = this.presets[activeId] || FALLBACK_PRESETS.neutral; const period = 3.2 + (Math.sin(seconds * 0.17) + 1) * 0.65; this.breathValue = this.breathEnabled ? (Math.sin((seconds / period) * Math.PI * 2) + 1) / 2 : 0; this.updateBlink(time); this.sampleLipSync(dt, seconds, preset); this.mouthOpen += (this.mouthTarget - this.mouthOpen) * (1 - Math.exp(-dt * 0.028)); this.mouthForm += (this.mouthFormTarget - this.mouthForm) * (1 - Math.exp(-dt * 0.022)); this.pointer.x += (this.pointerTarget.x - this.pointer.x) * (1 - Math.exp(-dt * 0.008)); this.pointer.y += (this.pointerTarget.y - this.pointer.y) * (1 - Math.exp(-dt * 0.008)); this.parameters = this.getParameterSnapshot(time); this.frameTimes.push(time); while (this.frameTimes.length && this.frameTimes[0] < time - 1000) this.frameTimes.shift(); this.fps = this.frameTimes.length; if (this.transition && clamp((time - this.transition.start) / this.transition.duration) >= 1) { const resolver = this.transition.resolve; this.currentEmotion = this.transition.to; this.currentIntensity = this.transition.toIntensity; this.emotionStartedAt = time; this.transition = null; resolver?.(); } }

  buildParameters(emotionId, intensity, time) { const seconds = time / 1000; const preset = this.presets[emotionId] || FALLBACK_PRESETS.neutral; const expression = expressionAt(preset.expression, intensity); const breathe = this.breathValue * (preset.breathScale ?? 1); const gazeX = this.pointer.x; const gazeY = this.pointer.y; const spokenMouthForm = lerp(expression.mouthForm || 0, this.mouthForm, clamp01(this.mouthOpen * 1.25)); return { ParamAngleX: gazeX * 5.5 * intensity, ParamAngleY: -gazeY * 3.8 * intensity, ParamAngleZ: (preset.pose?.rotation || 0) + gazeX * 1.2, ParamBodyAngleX: Math.sin(seconds * 0.7) * 0.45 * intensity, ParamBodyAngleY: breathe * 0.7 - 0.35, ParamBodyAngleZ: Math.sin(seconds * 0.53) * 0.32 * intensity, ParamEyeLOpen: clamp01((expression.eyeOpen ?? 1) * this.blinkLevel), ParamEyeROpen: clamp01((expression.eyeOpen ?? 1) * this.blinkLevel), ParamEyeLSmile: expression.eyeSmile || 0, ParamEyeRSmile: expression.eyeSmile || 0, ParamEyeBallX: clamp((expression.eyeBallX || 0) + gazeX * 0.48, -1, 1), ParamEyeBallY: clamp((expression.eyeBallY || 0) - gazeY * 0.35, -1, 1), ParamBrowLY: expression.browL || 0, ParamBrowRY: expression.browR || 0, ParamBrowLAngle: expression.browL || 0, ParamBrowRAngle: expression.browR || 0, ParamBrowLForm: expression.browL || 0, ParamBrowRForm: expression.browR || 0, ParamMouthOpenY: this.mouthOpen, ParamMouthForm: spokenMouthForm, ParamCheek: expression.cheek || 0, ParamBreath: breathe, ParamTwinTailL: Math.sin(seconds * 1.3) * breathe, ParamTwinTailR: Math.sin(seconds * 1.3 + 0.42) * breathe }; }
  ...
  drawEyeSystem(emotionId, source, params, alpha) { const layout = FACE_LAYOUTS[source]; if (!layout) return; const open = clamp01((params.ParamEyeLOpen + params.ParamEyeROpen) * 0.5); const closedAlpha = clamp01((1 - open) * 1.35) * alpha; const specialOpen = clamp01(open) * alpha; if (source === "uruuru") { this.drawPart("eyeUruuruL", ...layout.leftEye, specialOpen); this.drawPart("eyeUruuruR", ...layout.rightEye, specialOpen); } else if (source === "haku") { this.drawPart("eyeOpenL", ...layout.leftEye, specialOpen); this.drawPart("eyeOpenR", ...layout.rightEye, specialOpen); } else if (source === "peace" && ["teasing", "smug"].includes(emotionId)) this.drawPart("eyeWinkR", ...layout.rightEye, 0.88 * alpha); if (closedAlpha > 0.01 && source !== "gorogoro") { this.drawPart("eyeClosedL", ...layout.leftEye, closedAlpha); this.drawPart("eyeClosedR", ...layout.rightEye, closedAlpha); } }
  ...
  drawEffects(preset, sourceW, sourceH, alpha, time) { if (!preset.effects?.length) return; const ctx = this.ctx; const seconds = time / 1000; if (preset.effects.includes("rainbow") && this.parts.rainbow) this.drawPart("rainbow", -sourceW * 0.22, -sourceH * 0.08, sourceW * 0.44, sourceH * 0.43, (0.55 + 0.25 * Math.sin(seconds * 3.1)) * alpha); if (preset.effects.includes("sparkle")) { ctx.save(); ctx.globalAlpha *= alpha; ctx.fillStyle = "rgba(255, 224, 244, .9)"; for (let i = 0; i < 5; i += 1) { const phase = seconds * (0.9 + i * 0.07) + this.effectSeed + i * 1.7; const px = Math.sin(phase * 1.3) * sourceW * 0.42; const py = -sourceH * 0.2 + Math.cos(phase) * sourceH * 0.28; const size = 1.5 + (0.5 + 0.5 * Math.sin(phase * 3)) * 2; ctx.beginPath(); ctx.moveTo(px, py - size * 2); ctx.lineTo(px + size * 0.65, py - size * 0.65); ctx.lineTo(px + size * 2, py); ctx.lineTo(px + size * 0.65, py + size * 0.65); ctx.lineTo(px, py + size * 2); ctx.lineTo(px - size * 0.65, py + size * 0.65); ctx.lineTo(px - size * 2, py); ctx.lineTo(px - size * 0.65, py - size * 0.65); ctx.closePath(); ctx.fill(); } ctx.restore(); } if (preset.effects.includes("tears")) { ctx.save(); ctx.globalAlpha *= alpha * (0.55 + 0.25 * Math.sin(seconds * 2.2)); ctx.fillStyle = "rgba(171, 220, 255, .82)"; const layout = FACE_LAYOUTS[preset.source] || FACE_LAYOUTS.uruuru; for (const eye of [layout.leftEye, layout.rightEye]) { const x = eye[0] + eye[2] * 0.52; const y = eye[1] + eye[3] * 0.9 + ((seconds * 22) % 13); ctx.beginPath(); ctx.ellipse(x, y, 2.2, 5.6, 0, 0, Math.PI * 2); ctx.fill(); } ctx.restore(); } }

  getSnapshot() { return { emotion: this.transition ? this.transition.to : this.currentEmotion, transition: this.transition ? { from: this.transition.from, to: this.transition.to } : null, mouthOpen: this.mouthOpen, mouthTarget: this.mouthTarget, mouthForm: this.mouthForm, viseme: this.viseme, lipSyncMode: this.lipSyncMode, audio: { ...this.audioResult }, blinkLevel: this.blinkLevel, breath: this.breathValue, fps: this.fps, parameters: this.parameters, missingParts: Object.keys(PART_FILES).filter((key) => !this.parts[key]) }; }
  async destroy() { this.stop(); this.resizeObserver?.disconnect(); if (!this.resizeObserver) window.removeEventListener("resize", this.onWindowResize); this.canvas.removeEventListener("pointermove", this.onPointerMove); this.canvas.removeEventListener("pointerleave", this.onPointerLeave); await this.stopMicrophoneLipSync(); }
}
