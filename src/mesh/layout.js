...
export const FACE_LAYOUTS = {
  ...
  // gorogoro is a diagonal lying pose; these anchors are centered on its face and rotated locally by face-rig.js.
  gorogoro: { leftEye: [-26, -4, 32, 24], rightEye: [23, 18, 32, 24], mouth: [-11.5, 26, 31, 22] },
};

export function rectToNormalized(rect, sourceWidth, sourceHeight) {
  if (!rect || !sourceWidth || !sourceHeight) return null;
  const [x, y, width, height] = rect;
  return [(x / sourceWidth) * 2,(y / sourceHeight) * 2,(width / sourceWidth) * 2,(height / sourceHeight) * 2];
}
export function normalizedToRect(rect, sourceWidth, sourceHeight) {
  if (!rect || !sourceWidth || !sourceHeight) return null;
  const [x, y, width, height] = rect;
  return [x * sourceWidth * 0.5,y * sourceHeight * 0.5,width * sourceWidth * 0.5,height * sourceHeight * 0.5];
}
export function blendNormalizedRect(fromRect, fromSize, toRect, toSize, t) {
  ...
  const a=rectToNormalized(fromRect,fromSize[0],fromSize[1]); const b=rectToNormalized(toRect,toSize[0],toSize[1]); const p=Math.max(0,Math.min(1,Number(t)||0));
