...
import { GENERATED_REFERENCE_MOUTH_RECTS, hybridReferenceWeights, resolveGeneratedMouthRect, validateGeneratedReferenceConfig } from '../src/mesh/generated-reference-rig.js';
import { GENERATED_REFERENCE_PACK_EMOTIONS } from '../src/mesh/generated-reference-pack.js';
...
const generated = new Set(GENERATED_REFERENCE_PACK_EMOTIONS);
assert.equal(validateGeneratedReferenceConfig().length, 0, 'generated reference config must be valid');
assert.equal(generated.size, 9, 'nine verified generated reference endpoints are expected in alpha.1');
assert(!generated.has('sad'), 'sad intentionally uses articulated fallback in alpha.1');
...
for (const emotion of emotions) {
  ...
  for (const viseme of visemes) {
    ...
    const rect = generated.has(emotion)
    ...
    const [x,y,w,h] = rect;
    if (generated.has(emotion)) {
      assert(GENERATED_REFERENCE_MOUTH_RECTS[emotion], `${emotion}: generated mouth base missing`);
      assert(x >= 0 && y >= 0 && x+w <= 1 && y+h <= 1, `${emotion}/${viseme}: generated mouth out of bounds`);
...
console.log('hybrid 9 generated endpoints + 7 articulated fallbacks + 16 emotion lip-sync QA passed');
